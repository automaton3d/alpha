...
\begin{enumerate}
  ...
  \item \textbf{Conditional contact ratio.} $\alpha_B$ as in
        \eqref{eq:alpha}. The reference run at the campaign state yielded the proto-value
        $18/6996 \approx 1/388.7$ \cite{itfrombit}; at the current kernel the
        same run has no $s2B$ pass at the reference modulus
        (Table~\ref{tab:alphaA}); the task is its $L$-dependence.
...
complementary charges).  At the campaign state that matched [1] (commit
\texttt{5adede3}; log in \texttt{experiments/RESULTS.md}) the encounter
counters reproduced the manuscript exactly:
\begin{center}
...
quantitative null result of [1].  After the kernel's inertia corrections
(commits \texttt{e0048ca} and \texttt{381c1b4}) the same run at the current
HEAD re-measures to (9 Sep 2026; re-run log in
\texttt{experiments/RESULTS.md})
\begin{center}
  \texttt{active-passes} $= 6468$, \quad \texttt{s2B-passes} $= 0$, \quad
  \texttt{pairs-formed} $= 0$,
\end{center}
so the realized throughput at the reference modulus is zero there.
Bit-for-bit reproduction of a reference is the first check any measurement
must pass; the tables below therefore state the kernel state they were
recorded at.
...
\hline
 7 & 200 & 6468  &  0 & 0.003756878 & 266.18 \\
 9 & 200 & 7700  &  0 & 0.003739413 & 267.42 \\
11 & 200 & 12382 & 32 & 0.009334646 & 107.13 \\
13 & 100 & 10520 & 80 & 0.009865940 & 101.36 \\
\hline
\end{tabular}
\end{table}
...
\emph{not} constant in $L$ ($266.18, 267.42, 107.13, 101.36$, re-measured
at the current kernel). Under the
...
$400$ frames; recorded at the campaign state) gave $1/\alpha_A = 272.9$,
again frame-dependent and far from $1/137.036$.  At the current kernel the
realized throughput at $L=7$ is zero (no $s2B$ pass in the re-run), while
the theoretical throughput $1/\alpha_A(L{=}7) = 1/266.18$ remains within a
factor of $\sim$2 of $1/137.036$, so the search space is not empty; the remaining candidates of
...
and $\langle pB\rangle$ at each light frame.  At the current kernel the
controlled two-bubble probe never populates either bit at any tested size:
the reconstructed pair never appears ($n_{\mathrm{pol}}=0$;
$\langle pB\rangle=\langle sB\rangle=0$ at $L=7,9,11,13$), because the
polarization broadcast does not bootstrap in this probe after the inertia
corrections.  At the campaign state (commit \texttt{5adede3}) the candidate
was already rejected as unstable and non-monotonic: by total counts $0$,
$3.44$, $0.56$, $0.81$ at $L=7,9,11,13$, with the transverse bit never set
at $L=7$ and ratios at $L\ge9$ dominated by frames with tiny denominators.
Force-seeding an axis (the experimental \texttt{pol} bootstrap of
\texttt{alpha\_probe}, i.e.\ \texttt{polarization::seedAxis}) does
reconstruct the pair, but only on a single frozen phase branch of the
broadcast: $\langle sB\rangle/\langle pB\rangle=0$ at $L=11$ ($R=3$) and
exactly $1$ at $L=15$ ($R=5$)---degenerate, never near $1/137$.  The
canonical Platonic-seed configuration ($L=5$, $W=3L^2=75$, $400$ light
frames; campaign state) likewise never populates the magnetic channel, and
with all source centres superposed it produces no encounter overlap at all.
Candidate $\alpha_D$ as defined in Section~\ref{sec:candidates} is therefore
not measurable in any geometry tested so far.
...
$\alpha_A$: $\langle P(u,S)\rangle$ frame-averaged
          & $1/266\ldots 1/101$ ($L=7\ldots 13$, current kernel); $1/273$ canonical $L=5$ (campaign state) \\
$\alpha_B$: force events per contact
          & no collapse in current probes: realized $0/6468$ at $L=7$; gate passes form pairs ($L\ge11$: pairs $=$ s2B); campaign state read $1/388.7$ \\
...
$\alpha_D$: $\langle sB\rangle/\langle pB\rangle$
          & $0$ at every tested $L$ (no reconstructed pair); axis-seeded: $0$ or $1$ (degenerate) \\
$\alpha_E$: $\langle pol_v^2\rangle/\langle pol_u^2\rangle$
          & $\to 2$ exactly (analytic, $R\to\infty$) \\
$\alpha_F$: $U_0/S^*$, sieve fixed point
          & $S^*=\langle u\rangle$ re-measured: $41$ ($L=7$), $30$ ($L=9$) $\to \alpha_F \approx 50$, $68$ \\
\hline
\end{tabular}
\end{table}
...
\begin{itemize}
  \item \textbf{$\alpha_E$.} The transverse-to-radial energy ratio of the
        ...
        collapses to $pol_v\equiv0$.  In the current-kernel two-bubble probe
        the pair never reconstructs at any tested $L$ without the
        axis-seeded \texttt{pol} bootstrap, so this analytic ratio is not
        observable there.
  ...
  \item \textbf{$\alpha_F$.} The fixed point $S^*$ of the normalized
        ...
        $\langle u\rangle$, re-measured at the current kernel as $41$ at
        $L=7$ and $30$ at $L=9$, so $\alpha_F=U_0/S^*$ ($U_0=2048$) is
        geometry-locked ($\alpha_F\approx50$ and $68$).
  \item \textbf{$\alpha_B$.} At the campaign state read-only counters at the
        force-branch decisions showed every gate-passing contact collapsing
        (reissues): collapse/s2B $=1.0$ at the reference and $0.995$ with the
        channel open.  At the current kernel the probes read differently:
        the $L=7$ reference run has no gate pass (realized throughput
        $0/6468$), and the gate passes that do fire are consumed by
        complementary-pair formation (pairs $=$ s2B: $32/32$ at $L=11$,
        $80/80$ at $L=13$; with the channel open at $S=64$, $1{,}144$ pairs
        from $3{,}322$ passes at $L=7$ and $1{,}148$ from $3{,}952$ at
        $L=9$) while the collapse and repulsion counters stay zero.
        $\alpha_B$ therefore reduces to a pair-formation throughput that is
        $L$- and $S$-dependent and far from $1/137$, not to a collapse
        throughput.
\end{itemize}
...
consequence, not a signal.  At the current kernel the unseeded two-bubble
probe shows no reconstructed pair at any tested $L$ (the broadcast does not
bootstrap there), and the axis-seeded experimental bootstrap reconstructs
only a single frozen branch. (ii)~At the campaign state the two-bubble
contact was ``sticky''---the adiabatic (non-collapse) branch drifts sources
toward each other at every shell coincidence, so isolated bubbles never
separate at weak coupling and no clean pre/post collision window exists; the
current-kernel re-runs reproduce that only at small $L$ with a closed gate
($L\le9$, $S=16384$: $d$ constant over the run), while at $L\ge11$ the gate
opens and the pair recoils the bubbles (closest approach $d=1$ at $L=11$,
$d=0$ at $L=13$). (iii)~The initial momentum magnitude is inert: sources
advance one cell per tick regardless of $|m|$, so the emergent maximum
information speed cannot be exceeded by construction. (iv)~At the campaign
state the only registered force was sieve-gated collapse (every gate pass
reissued); at the current kernel the reference probes instead consume gate
passes by complementary-pair formation (pairs $=$ s2B at $L\ge11$, zero
passes at $L=7$), and repulsion counters remain zero; $\alpha_B$ therefore
reduces to a pair-formation throughput, not to collapse.
