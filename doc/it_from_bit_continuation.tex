...
budget), and the reference geometry $SEP=4$. Each run logs the convolution
...
complementary charges). The convolution counters reproduce the manuscript
exactly:
\begin{center}
...
population, frame-averaged (convolution counts are cumulative over the run).}
\label{tab:alphaA}
...
no convolution overlap at all ($conv\_calls=0$): the sources do not separate
